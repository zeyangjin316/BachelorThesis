...
def _save_dataframe_png(df: pd.DataFrame, png_path: str | None, head_n: int, dpi: int) -> str:
    """Render the head of a DataFrame to a width-fitted PNG (header-safe). Returns the saved path."""
    ...
    # Truncate cell text a bit more when many columns
    max_cell_chars = max(10, int(34 - 0.7 * ncols))
    df_for_png = df_for_png.applymap(lambda x: x if len(x) <= max_cell_chars else (x[: max_cell_chars - 1] + "…"))

    # Wrap headers at underscores; compute header line count
    pretty_cols = [str(c).replace("_", "\n") for c in df_for_png.columns]
    max_header_lines = max(lbl.count("\n") + 1 for lbl in pretty_cols) if ncols else 1

    # Figure size
    fig_w = max(14.0, min(60.0, 1.0 * ncols + 6.0))  # ~1 inch per column + padding
    ...
    # Table that fills the axes; equal column widths
    tbl = ax.table(
    ...
    # Extra vertical room for wrapped headers + slightly scale rows
    ...
    def visualize_ts(
        ...
        # Prepare panel: date x symbol (ret_crsp)
        ...
        # Dynamic sizing (stretched)
        n_dates = panel_sel.shape[0]
        n_syms = len(symbols)
        width_ts = max(16.0, min(36.0, n_dates / 40.0))  # wider with more dates
        height_ts = 6.5
        ...
        box = ax.get_position()
        ax.set_position([box.x0, box.y0, box.width * 0.82, box.height])  # reserve ~18% for legend
